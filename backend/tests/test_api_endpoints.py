...
class TestRollForwardEndpoint:
    ...
    def _rec(self, d, k, g, o, m):
        return {"dosya_no": d, "brans": "Yangın", "hasar_tarihi": k, "gelisim_tarihi": g, "odeme": o, "muallak": m}

    def test_roll_forward_appends_diagonal(self, client):
        resp = client.post("/v1/data/roll-forward", json={
            "prior_paid": self._PRIOR_PAID, "prior_incurred": self._PRIOR_INC,
            "records": [self._rec("B", "2022", "2024", 120, 90), self._rec("C", "2023", "2024", 260, 350), self._rec("D", "2024", "2024", 900, 600)],
            "brans": "Yangın",
        })
        assert resp.status_code == 200
        data = resp.json()
        assert data["paid_triangle"]["origin_periods"] == ["2021", "2022", "2023", "2024"]
        # 2023 age1 paid = 1200+260 = 1460
    ...
    def test_roll_forward_missing_brans_400(self, client):
        resp = client.post("/v1/data/roll-forward", json={
            "prior_paid": self._PRIOR_PAID, "records": [self._rec("X", "2023", "2024", 1, 0)], "brans": "Kasko",
        })
        assert resp.status_code == 400
