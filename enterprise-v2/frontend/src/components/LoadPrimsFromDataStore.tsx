    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="card w-full max-w-md shadow-xl border border-[color:var(--border)]">
        <div className="p-5 border-b border-[color:var(--border)] flex items-center justify-between">
          <h2 className="text-sm font-semibold">Load Premiums from Data Module</h2>
          ...
          <div>
            ...
            {store.periods.filter((p) => findPrim(p.datasets)).length === 0 ? (
              <p className="text-xs text-[color:var(--muted)]">
                No period with premium data found. Load it from the Data module.
              </p>
            ) : (
              <select
                value={periodId}
            ...
            <div>
              <div className="text-xs font-medium text-[color:var(--muted-strong)] mb-1.5">
                ...
                <span className="ml-2 font-normal text-[color:var(--muted)]">
                  {matchedCount}/{preview.length} periods matched
                </span>
              </div>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="bg-[color:var(--surface-alt)]">
                      <th className="px-3 py-1.5 text-left font-medium text-[color:var(--muted-strong)]">Period</th>
                      <th className="px-3 py-1.5 text-right font-medium text-[color:var(--muted-strong)]">EP</th>
                      <th className="px-3 py-1.5 text-center font-medium text-[color:var(--muted-strong)]">Match</th>
                    </tr>
                  </thead>
                  <tbody>
                    {preview.map((r) => (
        ...
        <div className="px-5 pb-5 flex justify-end gap-2">
          ...
          >
            {matchedCount > 0 ? `Load ${matchedCount} periods` : "Load"}
          </button>
        </div>
      </div>
    </div>
